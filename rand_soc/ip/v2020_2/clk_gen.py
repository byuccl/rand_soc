...
class ClkGen(IP):
    ...
    def instance(self):
        ...
        self.port_clk_out = self._create_hier_pin(
            "clk_out", Protocol.CLOCK, Direction.OUTPUT, 1
        )
        self.port_clk_out.connect_internal(f"{clk_gen_name}/clk_out1")

        self._create_hier_pin(
            "reset", Protocol.RESET, Direction.INPUT, 1
        ).connect_internal(f"{clk_gen_name}/reset")

        self.port_dcm_locked = self._create_hier_pin(
            "clk_locked", Protocol.CLOCK_LOCKED, Direction.OUTPUT, 1
        )
