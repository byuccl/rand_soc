...
class ClkGen(IP):
    ...
    def instance(self):
        ...
        self.port_clk_in = self._create_hier_pin("clk_in", Protocol.CLOCK, "I", 1)
        self.port_clk_in.connect_internal(f"{clk_gen_name}/clk_in1")

        self.port_clk_out = self._create_hier_pin("clk_out", Protocol.CLOCK, "O", 1)
        self.port_clk_out.connect_internal(f"{clk_gen_name}/clk_out1")

        self._create_hier_pin("reset", Protocol.RESET, "I", 1).connect_internal(
            f"{clk_gen_name}/reset"
        )

        self.port_dcm_locked = self._create_hier_pin(
            "clk_locked", Protocol.CLOCK_LOCKED, "O", 1
        )
